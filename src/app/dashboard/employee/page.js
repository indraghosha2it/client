...
// // app/dashboard/employee/page.js
// 'use client';
...
//     workingDays: 26,
//     absentDays: 0,
//     maidFee: 500, // NEW FIELD: Default 500
//     foodCost: 0, // NEW FIELD: Food cost
...
//           maidFee: employee.maidFee || 500, // Load maid fee
//           foodCost: employee.foodCost || 0, // Load food cost
...
//           workingDays: 26,
//           absentDays: 0,
//           maidFee: 500, // Reset to default
//           foodCost: 0, // Reset to 0
//           paymentMethod: "Bank Transfer",
//           notes: "",
//           salaryDate: new Date().toISOString().split('T')[0]
//         });
...
//   // Calculate per day salary and deductions (including maid fee and food cost)
//   const calculateSalaryDetails = (employee) => {
//     const perDaySalary = employee.salary / employee.workingDays;
//     const absenceDeduction = perDaySalary * employee.absentDays;
//     const maidFee = employee.maidFee || 0;
//     const foodCost = employee.foodCost || 0;
//     const otherDeductions = maidFee + foodCost;
//     const totalDeductions = absenceDeduction + otherDeductions;
//     const finalSalary = Math.max(0, employee.salary - totalDeductions);
//     const afterAbsenceAmount = employee.salary - absenceDeduction;
    
//     return {
//       perDaySalary: perDaySalary.toFixed(2),
//       absenceDeduction: absenceDeduction.toFixed(2),
//       maidFee: maidFee.toFixed(2),
//       foodCost: foodCost.toFixed(2),
//       otherDeductions: otherDeductions.toFixed(2),
//       totalDeductions: totalDeductions.toFixed(2),
//       finalSalary: finalSalary.toFixed(2),
//        afterAbsenceAmount: afterAbsenceAmount.toFixed(2)
//     };
//   };
...
//   // Calculate statistics for filtered employees (including maid fee and food cost)
...
//     const totalMaidFee = filteredEmployees.reduce((sum, emp) => sum + (emp.maidFee || 0), 0);
//     const totalFoodCost = filteredEmployees.reduce((sum, emp) => sum + (emp.foodCost || 0), 0);
    
//     const totalAbsenceDeductions = filteredEmployees.reduce((sum, emp) => {
//       const perDaySalary = emp.salary / emp.workingDays;
//       const absenceDeduction = perDaySalary * emp.absentDays;
//       return sum + absenceDeduction;
//     }, 0);
    
//     const totalOtherDeductions = totalMaidFee + totalFoodCost;
//     const totalDeductions = totalAbsenceDeductions + totalOtherDeductions;
//     const totalFinalSalary = totalSalary - totalDeductions;
    
//     return {
//       totalEmployees: filteredEmployees.length,
//       totalSalary: totalSalary.toFixed(2),
//       totalAbsenceDeductions: totalAbsenceDeductions.toFixed(2),
//       totalMaidFee: totalMaidFee.toFixed(2),
...
//  // Generate PDF report (updated for new fields)
// const generatePDF = () => {
...
//         employee.absentDays,
//         `BDT ${salaryDetails.absenceDeduction}`,
//         `BDT ${salaryDetails.maidFee}`,
//         `BDT ${salaryDetails.foodCost}`,
//         `BDT ${salaryDetails.totalDeductions}`,
...
//     autoTable(doc, {
//       startY: 50,
//       head: [['Employee Details', 'Salary Date', 'Base Salary', 'Work Days', 'Absent', 'Absence Ded', 'Maid Fee', 'Food Cost', 'Total Ded', 'Final Salary', 'Payment', 'Notes']],
//       body: tableData,
//       headStyles: {
...
//     const summaryStats = calculateFilteredStats();
    
//     // CHANGED: Reordered the summary items as requested
//     let yPos = lastY + 8;
    
//     // Row 1: Total Employees
//     doc.setTextColor(60, 60, 60);
//     doc.text("Total Employees:", 14, yPos);
//     doc.setFont("helvetica", "bold");
//     doc.setTextColor(0, 0, 0);
//     doc.text(summaryStats.totalEmployees.toString(), 70, yPos);
    
//     // Row 2: Total Base Salary
//     yPos += 8;
//     doc.setFont("helvetica", "normal");
//     doc.setTextColor(60, 60, 60);
//     doc.text("Total Base Salary:", 14, yPos);
//     doc.setFont("helvetica", "bold");
//     doc.setTextColor(0, 0, 0);
//     doc.text(`BDT ${summaryStats.totalSalary}`, 70, yPos);
    
//     // Row 3: Total Working Days (NEW - moved up)
//     yPos += 8;
//     doc.setFont("helvetica", "normal");
//     doc.setTextColor(60, 60, 60);
//     doc.text("Total Working Days:", 14, yPos);
//     doc.setFont("helvetica", "bold");
//     doc.setTextColor(0, 0, 0);
//     doc.text(summaryStats.totalWorkingDays.toString(), 70, yPos);
    
//     // Row 4: Total Absent Days (NEW - moved up)
//     yPos += 8;
//     doc.setFont("helvetica", "normal");
//     doc.setTextColor(60, 60, 60);
//     doc.text("Total Absent Days:", 14, yPos);
//     doc.setFont("helvetica", "bold");
//     doc.setTextColor(0, 0, 0);
//     doc.text(summaryStats.totalAbsentDays.toString(), 70, yPos);
    
//     // Row 5: Total Absence Deductions
//     yPos += 8;
//     doc.setFont("helvetica", "normal");
//     doc.setTextColor(60, 60, 60);
//     doc.text("Total Absence Deductions:", 14, yPos);
//     doc.setFont("helvetica", "bold");
//     doc.setTextColor(255, 0, 0); // Red for deductions
//     doc.text(`BDT ${summaryStats.totalAbsenceDeductions}`, 70, yPos);
    
//     // Row 6: Total Maid Fee
//     yPos += 8;
//     doc.setFont("helvetica", "normal");
//     doc.setTextColor(60, 60, 60);
//     doc.text("Total Maid Fee:", 14, yPos);
//     doc.setFont("helvetica", "bold");
//     doc.setTextColor(255, 0, 0); // Red for deductions
//     doc.text(`BDT ${summaryStats.totalMaidFee}`, 70, yPos);
    
//     // Row 7: Total Food Cost
//     yPos += 8;
//     doc.setFont("helvetica", "normal");
//     doc.setTextColor(60, 60, 60);
//     doc.text("Total Food Cost:", 14, yPos);
//     doc.setFont("helvetica", "bold");
//     doc.setTextColor(255, 0, 0); // Red for deductions
//     doc.text(`BDT ${summaryStats.totalFoodCost}`, 70, yPos);
    
//     // Row 8: Total Other Deductions
//     yPos += 8;
//     doc.setFont("helvetica", "normal");
//     doc.setTextColor(60, 60, 60);
//     doc.text("Total Other Deductions:", 14, yPos);
//     doc.setFont("helvetica", "bold");
//     doc.setTextColor(255, 0, 0); // Red for deductions
//     doc.text(`BDT ${summaryStats.totalOtherDeductions}`, 70, yPos);
    
//     // Row 9: Total Deductions
//     yPos += 8;
//     doc.setFont("helvetica", "normal");
//     doc.setTextColor(60, 60, 60);
...
//     doc.setFont("helvetica", "bold");
//     doc.setTextColor(255, 0, 0); // Red for deductions
//     doc.text(`BDT ${summaryStats.totalDeductions}`, 70, yPos);
    
//     // Row 10: Total Payable
//     yPos += 8;
//     doc.setFont("helvetica", "normal");
//     doc.setTextColor(60, 60, 60);
//     doc.text("Total Payable:", 14, yPos);
//     doc.setFont("helvetica", "bold");
//     doc.setTextColor(0, 128, 0); // Green for payable
//     doc.text(`BDT ${summaryStats.totalFinalSalary}`, 70, yPos);
...
//                   type="number"
//                   name="maidFee"
//                   placeholder="Enter maid fee"
//                   value={formData.maidFee}
//                   onChange={handleChange}
//                   className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
//                   min="0"
//                   step="0.01"
...
//             {/* Salary Calculation Preview (Updated) */}
...
//                   </div>
//                   <div className="bg-white p-2 rounded">
//                     <div className="text-gray-600">Absence Deduction</div>
...
//   <div className="font-semibold text-red-600">
//     -BDT {((parseFloat(formData.maidFee) || 0) + (parseFloat(formData.foodCost) || 0)).toFixed(2)}
//     <div className="text-xs text-gray-500">
//       (Maid: {parseFloat(formData.maidFee) || 0}, Food: {parseFloat(formData.foodCost) || 0})
//     </div>
//   </div>
// </div>
//                   <div className="bg-white p-2 rounded">
...
//                         (((parseFloat(formData.salary) || 0) / (parseInt(formData.workingDays) || 26)) * (parseInt(formData.absentDays) || 0)) -
//                         (parseFloat(formData.maidFee) || 0) -
...
//               </div>
//               <div className="bg-white p-2 rounded border">
//                 <div className="text-gray-600">Absence Ded</div>
//                 <div className="font-semibold text-red-600">BDT {stats.totalAbsenceDeductions}</div>
//               </div>
//               <div className="bg-white p-2 rounded border">
//                 <div className="text-gray-600">Other Ded</div>
//                 <div className="font-semibold text-red-600">BDT {stats.totalOtherDeductions}</div>
//               </div>
//               <div className="bg-white p-2 rounded border">
//                 <div className="text-gray-600">Total Ded</div>
...
//                       </th>
//                       <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
//                         Salary Details (with deductions)
//                       </th>
//                       <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
//                         Actions
//                       </th>
...
//                               {/* Absence Deduction */}
//                               <div className="flex justify-between">
//                                 <span className="text-sm text-gray-600">Absence Ded:</span>
//                                 <span className="text-sm text-red-600">-BDT {salaryDetails.absenceDeduction}</span>
//                               </div>
                              
//                               {/* Separator Line */}
//                               <div className="border-t border-dashed border-gray-300 pt-1">
//                                 <div className="flex justify-between">
//                                   <span className="text-sm font-medium text-gray-700">After Absence Ded:</span>
//                                   <span className="text-sm font-medium text-blue-600">
//                                     BDT {(employee.salary - parseFloat(salaryDetails.absenceDeduction)).toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})}
//                                   </span>
//                                 </div>
//                               </div>
...
//                               {/* Other Deductions */}
//                               <div className="flex justify-between">
//                                 <span className="text-sm text-gray-600">Maid Fee:</span>
//                                 <span className="text-sm text-red-600">-BDT {salaryDetails.maidFee}</span>
//                               </div>
                              
//                               <div className="flex justify-between">
//                                 <span className="text-sm text-gray-600">Food Cost:</span>
//                                 <span className="text-sm text-red-600">-BDT {salaryDetails.foodCost}</span>
//                               </div>
...
//                                     BDT {parseFloat(salaryDetails.finalSalary).toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})}
//                                   </span>
//                                 </div>
//                               </div>
...
//               {/* Salary Summary Section (Updated) */}
//               {filteredEmployees.length > 0 && (
...
//                         </div>
//                         <div>
//                           <div className="text-sm text-gray-600">Absence Ded</div>
//                           <div className="text-2xl font-bold text-red-600">BDT {stats.totalAbsenceDeductions}</div>
//                         </div>
//                         <div>
//                           <div className="text-sm text-gray-600">Other Ded</div>
//                           <div className="text-2xl font-bold text-red-600">BDT {stats.totalOtherDeductions}</div>
//                         </div>
//                         <div>
//                           <div className="text-sm text-gray-600">Total Ded</div>
...
//                   {/* Additional Stats */}
//                   <div className="grid grid-cols-1 md:grid-cols-5 gap-4 mt-4 pt-4 border-t border-green-200">
//                     <div>
//                       <div className="text-sm text-gray-600">Maid Fee</div>
//                       <div className="font-semibold">BDT {stats.totalMaidFee}</div>
//                     </div>
//                     <div>
//                       <div className="text-sm text-gray-600">Food Cost</div>
//                       <div className="font-semibold">BDT {stats.totalFoodCost}</div>
//                     </div>
//                     <div>
...
export default function EmployeePage() {
  ...
  const fetchEmployees = async () => {
    try {
      ...
      if (data.success) {
        setEmployees(data.data);
        setFilteredEmployees(data.data);
        // showToast(`Loaded ${data.count} employees`, 'success');
      } else {
    ...
    try {
      ...
      if (data.success) {
        ...
        setFormData({
          ...
          workingDays: employee.workingDays || 26,
          absentDays: employee.absentDays || 0,
          maidFee: employee.maidFee || 500,
          foodCost: employee.foodCost || 0,
          paymentMethod: employee.paymentMethod || "Bank Transfer",
          notes: employee.notes || "",
          salaryDate: employee.salaryDate ? new Date(employee.salaryDate).toISOString().split('T')[0] : new Date().toISOString().split('T')[0]
        });
        ...
        showToast(data.message || 'Failed to load employee data', 'error');
      }
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        const actionText = isEditMode ? "updated" : "added";
        showToast(`Employee ${actionText} successfully!`);
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      const response = await fetch(`${API_URL}/employees/${deleteConfirmation.employeeId}`, {
        method: "DELETE",
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('auth_token')}`,
        },
      ...
      if (data.success) {
        ...
        if (editingId === deleteConfirmation.employeeId) {
          cancelEdit();
        }
        fetchEmployees();
      } else {
  ...
  const generatePDF = () => {
    if (filteredEmployees.length === 0) {
      showToast('No employees data to download', 'error');
      return;
    }
              ...
              <div className="flex items-start space-x-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600 mt-1">
                    Are you sure you want to delete <span className="font-medium">{deleteConfirmation.employeeName}</span> ({deleteConfirmation.employeeDesignation})? This action cannot be undone.
                  </p>
                </div>
              </div>
              <button
